...
export function SceneLighting() {
  return (
    <>
      ...
      <ambientLight intensity={0.5} color={palette.hearth} />

      {/* Warm key — the hearth above the table. */}
      <pointLight position={[0.5, 6, 2.5]} intensity={1.25} color={palette.hearthSoft} distance={22} decay={1.4} />

      {/* Cool rim/fill from low and behind for depth and a touch of magic. */}
      <pointLight position={[-4, 2, -4]} intensity={0.65} color={palette.arcane} distance={18} decay={1.6} />

      {/* Warm bounce from the felt. */}
      <hemisphereLight args={[palette.hearth, palette.feltDeep, 0.35]} />
